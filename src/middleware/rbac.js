...
async function getProjectRole(userId, projectId) {
  const row = await queryOne(
    `SELECT p.org_id, om.role AS org_role, pm.role AS project_role
     FROM projects p
  ...
  if (!row) return null;
  const ranks = [ROLE_RANK[row.org_role], ROLE_RANK[row.project_role]].filter((r) => r !== undefined);
  if (!ranks.length) return null;
  return Object.keys(ROLE_RANK).sort((a, b) => ROLE_RANK[b] - ROLE_RANK[a]).find((r) => ROLE_RANK[r] === Math.max(...ranks));
}
