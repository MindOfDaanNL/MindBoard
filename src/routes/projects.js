...
router.post('/', requireOrgRole('member'), async (req, res, next) => {
  try {
    ...
    const projectId = await transaction(async (conn) => {
      ...
      await conn.query('INSERT INTO project_members (project_id, user_id, role) VALUES (?, ?, ?)', [id, req.user.id, 'admin']);
...
router.get('/:projectId', requireProjectRole('viewer'), async (req, res, next) => {
  try {
    ...
    const members = await query(
      `SELECT DISTINCT u.id, u.email, u.username, u.full_name, u.avatar_color, u.avatar_url, u.status,
...
// GET /api/projects/:projectId/metrics — agile statistieken voor het project
router.get('/:projectId/metrics', requireProjectRole('viewer'), async (req, res, next) => {
  try {
    const projectId = Number(req.params.projectId);
    const doneRe = /klaar|done|afgerond|gereed/i;
    ...
    const byColumn = await query(
      `SELECT c.name, c.color, COUNT(t.id) AS n
       ...
       WHERE b.project_id = ?
       GROUP BY c.id ORDER BY c.position ASC, c.id ASC`,
      [projectId]
    );

    const byPriority = await query(
      `SELECT t.priority, COUNT(*) AS n FROM tasks t
       JOIN boards b ON b.id = t.board_id WHERE b.project_id = ? GROUP BY t.priority`,
      [projectId]
    );

    const byAssignee = await query(
      `SELECT u.id, u.username, u.full_name, u.avatar_color, COUNT(t.id) AS n
       FROM users u
       JOIN org_members om ON om.user_id = u.id AND om.org_id = (SELECT org_id FROM projects WHERE id = ?)
       LEFT JOIN tasks t ON t.assignee_id = u.id AND t.board_id IN (SELECT id FROM boards WHERE project_id = ?)
       GROUP BY u.id ORDER BY n DESC, u.full_name ASC LIMIT 10`,
      [projectId, projectId]
    );
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      series.push({ date: key, created: createdMap[key] || 0, done: doneMap[key] || 0 });
    }
    ...
    const total = await queryOne(
      'SELECT COUNT(*) AS n FROM tasks WHERE board_id IN (SELECT id FROM boards WHERE project_id = ?)',
      [projectId]
    );
    const doneCount = doneColIds.length
      ? (await queryOne(`SELECT COUNT(*) AS n FROM tasks WHERE column_id IN (${doneColIds.map(() => '?').join(',')})`, doneColIds)).n
      : 0;
    ...
    return res.json({
      ...
      byAssignee,
      series,
      totals: { tasks: total.n, done: doneCount, open: total.n - doneCount }
    });
  } catch (e) {
    return next(e);
  }
