...
CREATE TABLE tasks (
  ...
  recurrence_next_at    DATETIME DEFAULT NULL,
...
-- ----------------------------------------------------------------------------
-- Notificatievoorkeuren (per gebruiker, per type)
-- ----------------------------------------------------------------------------
CREATE TABLE notification_prefs (
  user_id INT UNSIGNED NOT NULL,
  type    ENUM('assignment','comment','mention','info') NOT NULL,
  enabled TINYINT(1) NOT NULL DEFAULT 1,
  PRIMARY KEY (user_id, type),
  CONSTRAINT fk_notifprefs_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;